// LR8_2 fixed.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
#include <iostream>
#include <string>
using namespace std;
...
int find_pos(list_arr<T, size>& LA, T val) {
	int counter = -1;
	int counter2 = LA.true_size;
	for (int i = -1; i < LA.true_size; i++) {
		if (LA.value[i] == val) {
			counter = i;			
			break;
		}
	...
	cout << counter;
	return counter; // -1, if wrong value; 0, if element`s position = 0;
}
